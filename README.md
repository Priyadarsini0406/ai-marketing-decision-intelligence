...
The authenticated dashboard links to `/dashboard/leads`, `/dashboard/analytics`, and `/dashboard/budget`. These pages read stored leads/predictions, channel metrics, and saved simulations from the API and show empty states when records are unavailable. Overview export downloads stored channel and simulation data; its displayed sample metrics are explicitly labeled demonstration data. Conversion model training is available through `backend/train_model.py` (see [training instructions](data/README.md)); creation of new simulations remains unavailable. Signing in after opening a protected dashboard page returns regular users to that page.
