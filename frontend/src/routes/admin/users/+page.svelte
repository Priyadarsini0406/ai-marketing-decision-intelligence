...
{#each filteredUsers as user}<tr><td>{user.name}</td><td>{user.email}</td><td>{user.role}</td><td>{user.active ? 'Active' : 'Inactive'}</td><td><button class="secondary" onclick={() => edit(user)}>Edit</button></td></tr>{:else}<tr><td colspan="5">{loading ? 'Loading users…' : 'No accounts match these filters.'}</td></tr>{/each}
</tbody></table></div></div>
